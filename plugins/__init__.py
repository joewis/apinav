...
NEVER persisted (Joerg ruling 2026-09-11: no scraping/dumping of plugin
sources). One module per source, uniform shape:

    search(query, limit) -> list[node_dict]  # apinav row shape + source tag

Shared behavior (see apisio_client.py for the original):
...
- apisio   — 133.6k APIs, q= search, docs from humanURL (apisio_client.py)
